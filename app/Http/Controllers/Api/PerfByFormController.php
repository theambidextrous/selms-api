...
class PerfByFormController extends Controller
{
    public function findbyform(Request $request)
    {
    ...
    public function downloadlist(Request $request)
    {
        ...
        $the_form_subjects = [];
        $the_form_stud_ids = Student::select('id')->where('form', $input['form'])->get();
        if(!is_null($the_form_stud_ids))
        {
            $the_form_stud_ids = $the_form_stud_ids->toArray();
            $theform_sub_ids = Enrollment::select('subject')->whereIn('student', $the_form_stud_ids)
                ->where('status', 'enrolled')
                ->where('year', $term_meta->year)
                ->get();
            if(!is_null($theform_sub_ids))
            {
                $theform_sub_ids = $this->filter_dup_ids($theform_sub_ids->toArray());
                foreach( $theform_sub_ids as $one ):
                    ...
                    array_push($the_form_subjects, $entry);
                endforeach;
            }
        }
        $f_meta = [
            'form' => Form::find($input['form'])->name,
            'cat' => $the_assess->name,
            'count' => $enroll_count,
        ...
        $pdf_data = [
            ...
            'marks' => $this->get_sub_str_Scores($the_form_subjects, $input, $the_form_stud_ids),
        ];
        $filename = ('app/cls/trt/content/' . $uuid_string);
        PDF::loadView('reports.formmarklist', $pdf_data, [], ['orientation' => 'L', 'margin_top' => 4, 'margin_right' => 15, 'margin_left' => 15, 'margin_bottom' => 5])->save(storage_path($filename));
        return response([
            'status' => 200,
