...
<title>Student Bulk - Progress Report</title>
<style>
#customers {
...
<body>
@if(count($bulk_pdfdata))
    @foreach( $bulk_pdfdata as $alldata )
    <header>
    ...
    <table rules="none" class="table" id="customers" width="1200px">
      <tr class="td-borderless" style="background-color: #ffffff !important;border:none;">
        <td width="350px" class="td-borderless" style="width:150px!important;border-color:#fff;">
            <img src="{{ storage_path( 'app/cls/trt/content/' . $alldata['setup']['logo'] ) }}" width="100px" alt="logo"/><br>
            <p class="p-label">{{ $alldata['setup']['school'] }}</p>
        </td>
  ...
  <!-- Main Content -->
    ...
    <table class="table" id="customers" width="800px">
        <tr>
            <td>Student. </td>
            <td><b>{{ $alldata['meta']['fullname'] }}</b></td>
            
            <td>Adm No. </td>
            <td><b>{{ $alldata['meta']['admission'] }}</b></td>
            
            <td>Form. </td>
            <td><b>{{ $alldata['meta']['flabel'] }}</b></td>

            <td>Stream. </td>
            <td><b>{{ $alldata['meta']['slabel'] }}</b></td>
            
        </tr>
        <tr>
            <td>KCPE. </td>
            <td><b>{{ $alldata['meta']['kcpe'] }}</b></td>

            <td>Co-curricular. </td>
            <td><b>{{ $alldata['meta']['sport'] }}</b></td>

            <td>Class teacher. </td>
            <td><b>{{ $alldata['meta']['cteacher'] }}</b></td>
            
            <td>Parent. </td>
            <td><b>{{ $alldata['meta']['parentname'] }}</b></td>
        </tr>
    </table>

    <h4 class="text-4 mt-2"><b>{{$alldata['meta']['termname']}} Performance</b></h4>
    <!-- SPlit parent -->
    <table id="content">
      <tr id="content-tr">
        ...
        <table class="table" id="customers" width="800px">
          <thead class="dark-head">
            <tr>
              ...
              @if( count($alldata['headers']) )
                @foreach( $alldata['headers'] as $head )
          ...
          <tbody>
            @if(count($alldata['marks']))
              @foreach( $alldata['marks'] as $value )
            ...
            @if(count($alldata['avr']))
              <tr>
                <td style="text-align:left;"><b>Average</b></td>
                @foreach( $alldata['avr'] as $average)
      ...
      <td id="content-td" style="width: 38%; text-align:right;">
        <img src="{{ storage_path($alldata['chart']) }}" width="350px" alt="trend"/><br>
      </td>
      </tr>
    </table>
    ...
    <table class="table" id="customers" width="800px">
      <tbody>
        <tr>
            ...
            <td><b>{{$alldata['last_perf']}}</b></td>
            <td><b>Performance Deviation</b></td>
            <td><b>{{ $alldata['deviation'] }}</b></td>
        </tr>
        <tr>
            <td style="text-align:left;"><b>Fee balance.</b></td>
            <td><b>Ksh.{{$alldata['fees']}}</b></td>
    ...
    <table class="table" id="customers" width="800px">
        <tr>
            <td>Grade</td>
            @foreach( $alldata['scale'][0] as $scl )
                <td>{{ $scl }}</td>
            @endforeach
        </tr>
        <tr>
            <td>Score</td>
            @foreach( $alldata['scale'][1] as $gscl )
                <td>{{ $gscl }}</td>
            @endforeach
        </tr>
    </table>
    ...
    <footer class="text-center">
        <br>
        <hr>
        <p class="text-center">{{ $alldata['setup']['school'] }} | {{$alldata['setup']['address']}} | {{ $alldata['setup']['county'] }} {{ $alldata['setup']['zip'] }} <br><i><span style="color:#036a6b;">{{ Config::get('app.name') }}. Web: {{ Config::get('app.url')}}</span> </i></p>
    </footer>
    <!-- </div> -->
